...
桌面版可以通过 AWS S3，或支持 `If-None-Match: *` 条件对象创建的 S3 兼容存储双向同步 Vault。设置中提供 Cloudflare R2、阿里云 OSS 和 MinIO 预设，但实际兼容性取决于服务版本与配置。Vault 同步按 Vault 单独配置，并且完全独立于图片存储：不会复用图片的 `S3_*` 配置，也不会混用图片对象。
...
同步协议 v1 会通过条件写入初始化新的空 prefix；多台首设备发生竞争时，只允许一个远端 manifest 成功创建。不支持或忽略 `If-None-Match: *` 的后端不能用于 Vault 同步。仍应等首台设备完成首次同步后再配置后续设备，避免首次文件上传相互竞争，也便于在服务兼容性或凭据有误时恢复。
